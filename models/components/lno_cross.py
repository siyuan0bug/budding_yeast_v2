...
class LaplaceConv1d(nn.Module):
    def __init__(self, in_channels, out_channels, modes1):
        ...
        self.sigma = nn.Parameter(torch.randn(1, in_channels, 1) * 0.02)

    def forward(self, x):
        batchsize, channels, time_steps = x.shape
        t = torch.linspace(0, 1, time_steps, device=x.device).view(1, 1, time_steps)
        sigma_clamped = torch.clamp(self.sigma, min=-5.0, max=5.0)
        envelope = torch.exp(-sigma_clamped * t)
        x_laplace = x * envelope
        x_ft = torch.fft.rfft(x_laplace)
...
class NIPSYeastBlock(nn.Module):
    def __init__(self, num_vars, width, modes, adj_matrix=None):
        ...
        self.norm_laplace = nn.GroupNorm(1, num_vars * width)
        self.norm_attn = nn.GroupNorm(1, num_vars * width)
        self.norm_local = nn.GroupNorm(1, num_vars * width)

        self.laplace_conv = LaplaceConv1d(num_vars * width, num_vars * width, modes)
    ...
    def forward(self, x, shift):
        ...
        x_laplace_in = self.norm_laplace(x_flat)
        x_lp = self.laplace_conv(x_laplace_in).reshape(B, V, W, T)

        x_attn_in = self.norm_attn(x_flat).reshape(B, V, W, T)
        x_lp_T = x_lp.permute(0, 3, 1, 2)
        ...
        return F.gelu(x_attn + x_lp + x_local + shift + x)
...
class NIPSYeastLNO(nn.Module):
    def __init__(self, num_vars=39, param_dim=141, modes=24, width=16, n_blocks=4, adj_matrix=None):
        ...
        self.blocks = nn.ModuleList([NIPSYeastBlock(num_vars, width, modes, adj_matrix) for _ in range(n_blocks)])
